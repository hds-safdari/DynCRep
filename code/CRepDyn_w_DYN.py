...
class CRepDyn_w_temp:
	def __init__(self, undirected=False, initialization=0, ag=1.5,bg=2., rseed=0, inf=10000000000.0, err_max=0.000000000001, err=0.01,
				 N_real=1, tolerance=0.1, decision=2, max_iter=1000, out_inference=False,
	...
	def _initialize_w(self,w0): 
		...
		for i in range(self.T):
			wloc = np.concatenate((wloc, self.w), axis=0)
		self.w = wloc.copy() 
	...
	def __Likelihood(self, data, data_T, data_T_vals, subs_nz,T, mask=None,EPS=1e-12):
		...
		if self.T > 0:
			l += (np.log(self.beta_hat[-1]+EPS) * self.bAtAtm1).sum()
