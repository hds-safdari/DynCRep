...
class CRepDyn:
	...
	def _update_U(self, subs_nz):
		...
		if not self.constrained:
			...
			Z_uk *= (1.+self.beta_hat[self.T] * self.T)
			non_zeros = Z_uk > 0. 
			self.u[:, Z_uk == 0] = 0.
	...
	def _update_V(self, subs_nz):
		...
		if not self.constrained:
			...
			Z_vk *= (1.+ self.beta_hat[self.T] * self.T )
			non_zeros = Z_vk > 0
			self.v[:, Z_vk == 0] = 0.
...
# 		if isinstance(data_AtAtm1, skt.dtensor):
# 			# to do
# 			bt = - (T) * lambda0_ija[sub_mask_nz].sum() - obj.eta * (data_T[sub_mask_nz] * obj.beta_hat[sub_mask_nz[0]]).sum()
# 		elif isinstance(data_AtAtm1, skt.sptensor): 
# 			bt = - (T) * lambda0_ija[sub_mask_nz].sum() - obj.eta * (data_T.toarray()[sub_mask_nz] * beta_t).sum()
# 	else:
# 		if isinstance(data_AtAtm1, skt.dtensor):
# 			# to do
# 			# bt = - (T) * lambda0_ija.sum() - obj.eta * ( data_T[0].sum() + obj.beta * data_T[1:].sum())
# 			bt = - (T) * lambda0_ija - obj.eta * ( data_T[0].sum() + obj.beta * data_T[1:].sum())
# 		elif isinstance(data_AtAtm1, skt.sptensor): 
# 			# bt =  - (T) * lambda0_ija.sum() - obj.eta * (data_T_vals).sum()  # adding -(1+beta*T)* sum uvw - beta*eta*Aji(t-1)
# 			bt =  - (T) * lambda0_ija - obj.eta * (data_T_vals).sum()  # adding -(1+beta*T)* sum uvw - beta*eta*Aji(t-1)

# 	bt +=  (data_AtAtm1.vals.sum()- obj.E0)/beta_t  # adding sum A_hat from 1 to T
# 	bt -=  obj.bAtAtm1/(1-beta_t)  # adding Aij(t-1)*Aij(t)
# 	bt += obj.Atm11At/beta_t  # adding Aij(t-1)*(1-Aij(t))
# 	return bt